...
const wasmArtifacts = files.filter((path) => /^dist\/wasm\/libassimp\.(?:js|wasm)$/u.test(path));
if (wasmArtifacts.length !== 2)
  throw new Error(`expected one glue/Wasm pair, found ${wasmArtifacts.length} files`);
...
const buildConfigSha256 = createHash('sha256')
  .update(readFileSync(new URL('../assimp-builds.json', import.meta.url)))
  .digest('hex');
const manifest = JSON.parse(
  readFileSync(new URL('../src/wasm/libassimp.manifest.json', import.meta.url), 'utf8'),
);
const mismatch = [
  ...
  manifest.buildConfigSha256 !== buildConfigSha256 && 'built from a different assimp-builds.json',
...
if (mismatch.length > 0) throw new Error(`libassimp: ${mismatch.join('; ')}`);
