...
The engine is compiled with `-O3 -DNDEBUG`, Closure-compiled glue
(`--closure=1`), `-sEVAL_CTORS=2` to run static constructors at build time,
...
Continuous integration reports every binary's raw, gzip and brotli sizes and
validates its WebAssembly section shape. The sizes on the [home page](/) are
measured from the same artifacts, not typed in by hand.
