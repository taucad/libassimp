...
Every entry ships one baseline-compatible Wasm artifact:
...
The import table is directional: it lists extensions each entry can read, not targets it can write.
...
All three variants compile and link with `-O3 -DNDEBUG`, fixed SIMD, mimalloc, Wasm exceptions, Closure-compiled glue, constructor evaluation, and standalone `wasm-opt -O4 --converge --traps-never-happen --skip-pass=code-folding`. The build explicitly pins `WASM_LEGACY_EXCEPTIONS=1`; exnref waits until every documented host floor supports it. LTO, relaxed SIMD, pthreads, Asyncify, and mode-specific JSPI artifacts remain absent.

Build manifests record exact definitions, compile/link/optimizer flags, import/export inventory, and raw/gzip/brotli size for each artifact. CI ratchets those measured files rather than estimating them.
